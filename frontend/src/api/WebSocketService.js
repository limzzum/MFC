...
const WebSocketService = () => {
    ...
    const connectChat = (roomId, onMessageReceived) => {
        ...
        chatStompClientRef.current.connect({}, () => {
            ...
            chatStompClientRef.current.subscribe(`/from/chat/${roomId}`, (message) => {
                const content = JSON.parse(message.body);
                onMessageReceived({ sender: content.nicknName, text: content.message});
            });
        });
    };
    ...
    const connectFile = (roomId, onFileReceived) => {
        ...
        fileStompClientRef.current({}, () => {
            ...
            fileStompClientRef.current.subscribe(`/from/room/file/${roomId}`, (message) => {
                const content = JSON.parse(message.body);
    ...
    const connectPlayer = (roomId, onPlayerReceived) => {
        ...
        playerStompClientRef.current({}, () => {
            ...
            playerStompClientRef.current.subscribe(`/from/room/player/${roomId}`, (message) => {
                const content = JSON.parse(message.body);
